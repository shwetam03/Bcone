    ...
    function uploadDocument(docElement, docType, GateaBRDNo) {
        ...
        function addItem(buffer, fileName, docType, GateaBRDNo) {
            ...
            call.done(function (data, textStatus, jqXHR) {
                ...
                call2.done(function (data, textStatus, jqXHR) {
                    var item = data.d;
					//alert('item'+item);
                    var callUpdate = updateItemFields(item, docType, GateaBRDNo,fileName);
        ...
        function updateItemFields(item, documentType, GateaBRDNo,fileName) {
          ...
          //alert(fileName);
            var call = jQuery.ajax({
                url: _spPageContextInfo.webAbsoluteUrl +
                    "/_api/Web/Lists/getByTitle('BconeBillAttachment')/Items(" +item.Id +")",
                type: "POST",
                data: JSON.stringify({
                    "__metadata": { type: "SP.Data.BconeBillAttachmentItem" },
                    ExpenseItemId: GateaBRDNo,
					filename:fileName
                }),
                headers: {
                    Accept: "application/json;odata=verbose",

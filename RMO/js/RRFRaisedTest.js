...
$(document).ready(function () {
   ...
   ({
    ...
    if (currentuserRole == "RMO") {
        projecturl = site_url + "_api/Projects";
    }//else if(currentuserRole){
    
        //}
    else {
        projecturl = site_url + "_api/Projects?$filter=ProjectOwnerId eq guid'" + currentuserid + "' or substringof('" + currentuser_EmployeeID + "',ClientPartner)";
    }
    ...
    if (rrfno != undefined || rrfno != null) {
        ...
        url = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('RRF')/Items?$select=ReportingToUser/Title,ReportingToUser/EMail,*&$expand=ReportingToUser&$filter=RRFNO eq '" + rrfno + "'";
        ...
        if (resultRRF[0].EmployeeRole != "Billable") {
            $("#btnsearch").prop("disabled", true);
        }
        ...
        if (resultRRF[0].Status == "RMO Validation" || resultRRF[0].Status == "RRF Saved" || resultRRF[0].Status == "Non Billable" || resultRRF[0].Status == "Functional Head") {
    ...
    for (var index = 0; index < resultvacancy.length; index++) {
        if (resultvacancy[index].Type == "EmployeeRoles") {
            if ($('#rdbnew').is(':checked')) {
                ...
                if (resultvacancy[index].Value == "Billable") {
                    //$("#ddlemployeerole").empty();
...
function displayData(result) {
    ...
    if (resultRRF[0].ProjectManager != null || resultRRF[0].ProjectManager != undefined) {
        var projectmanager = resultRRF[0].ProjectManager;
        projectmanager = projectmanager.split(";");
        projectmanager = projectmanager[0];
        $("#txtprojectmanager").val(projectmanager);
    }

    if (resultRRF[0].FPAApprover != null || resultRRF[0].FPAApprover != undefined) {
        var fpnaapprover = resultRRF[0].FPAApprover;
        fpnaapprover = fpnaapprover.split(";");
        fpnaapprover = fpnaapprover[0];
        $("#txtfpaapprover").val(fpnaapprover);
    }

    if (resultRRF[0].ClientPartner != null || resultRRF[0].ClientPartner != undefined) {
        var clientpartner = resultRRF[0].ClientPartner;
        clientpartner = clientpartner.split(";");
        clientpartner = clientpartner[0];
        $("#txtclientpartner").val(clientpartner);
    }

    if (resultRRF[0].RMOSpoc != null || resultRRF[0].RMOSpoc != undefined) {
        var rmospoc = resultRRF[0].RMOSpoc;
...
$("#ddlprojectname").change(function () {
    ...
    if ($('input[name=projectstatus]:checked').val() == "Existing") {
        ...
        var url = site_url + "_api/Projects?$filter=ProjectName eq '" + value + "'";
        ...
        $("#txtoppertunitystatus").val(result[0].DealDeskID);
        $("#txtcustomer").val(result[0].CustomerName);
        $("#txtgbu").val(result[0].GBU);
        if (result[0].FPNA != null) {
            fpna = result[0].FPNA;
            if (fpna.indexOf("|") > 0) {
                $("#txtfpaapprover").val(fpna.substr(fpna.lastIndexOf('|') + 1));
                var fpnaid = fpna.substring(0, fpna.lastIndexOf("|"));
                var urlfpna = "https://ppmdev.bcone.com/_api/Resources?$select=ResourceId,ResourceEmailAddress&$filter=EmployeeID eq '" + fpnaid + "'";
        ...
        if (result[0].RMOSPOC != null) {
            ...
            if (rmospoc.indexOf("|") > 0) {
                ...
                var urlrmospoc = "https://ppmdev.bcone.com/_api/Resources?$select=ResourceId,ResourceEmailAddress&$filter=EmployeeID eq '" + rmoid + "'";
        ...
        if (result[0].ClientPartner != null) {
            ...
            if (clientpartner.indexOf("|") > 0) {
                ...
                var urlcp = "https://ppmdev.bcone.com/_api/Resources?$select=ResourceId,ResourceEmailAddress&$filter=EmployeeID eq '" + cpid + "'";
...
$('#ddlemploymenttype').on('change', function () {
    ...
    var employetype = $('#ddlemploymenttype option:selected').text();
    var urllocation = "https://ppmdev.bcone.com/_api/RRFMaster?$filter=Type eq 'EmployeeTypeId' and Value eq '" + employetype + "'&$orderby=Code asc";
    var result = resultset(urllocation);
    $("#ddlemptypey").empty();
    $("#ddlemptypey").append(new Option("Select", 0));
    for (var index = 0; index < result.length; index++) {
...
function getSerachDetails() {
    ...
    var result = searchResource("NA", primaryskill, secondryskill, RoleBand, projectstartdate)
...
function bindtblResource(result) {
    ...
    table_header = table_header + "<th>EmpID</th>"
    table_header = table_header + "<th>Name</th>"
    table_header = table_header + "<th>Designation</th>"
    table_header = table_header + "<th>Location</th>"
...
function submitRRF() {
    ...
    setTimeout(function () {
        ...
        $('.loader').show();
        if ($('#ddlemployeerole option:selected').text() != "Billable" && $('#ddlemployeerole option:selected').text() != "Select") {
...
$("#ddlemployeerole").change(function (e) {
    ...
    if ($('#ddlemployeerole option:selected').text() != "Billable") {
        $("#btnsearch").prop("disabled", true);
        resourcenameRMO = "";
    }
    else {
...
$("#ddlroleband").change(function (e) {
    ...
    else if ($('#ddlemployeerole option:selected').text() == "Select" || $('#ddlemployeerole option:selected').text() == "Billable") {
        $("#btnsearch").prop("disabled", false);
    }
    // $("#btnsearch").prop("disabled", false);
...
function getAttchmentReason() {
    ...
    itemCreation.set_folderUrl('/sites/PPMUAT/RRFSupportDocument');//URL of the folder
...
function uploadFileLocal(file, RRFItemId, folder_name) {
    ...
    reader.onload = function (e) {
        ...
        url = String.format(
        "{0}/_api/web/GetFolderByServerRelativeUrl('/sites/PPMUAT/RRFSupportDocument/" + NewItemID + "')/Files/Add(url='{1}', overwrite=true)",

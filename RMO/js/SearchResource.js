...
function searchResource(Employee_ID, PrimarySkill, SecondrySkill, RolBand, projectStrtdate, Allocationvalue, ProjectEnddate) {
    if(PrimarySkill != undefined && PrimarySkill != null && PrimarySkill != "null")
	{		
		...
		while (PrimarySkill.indexOf('+') >= 0 )
		{
			//PrimarySkill = PrimarySkill.replace('+', '%2B'); old one
			PrimarySkill = PrimarySkill.replace(/[+]/g,'%2B');
		}
	}
	if(SecondrySkill != undefined && SecondrySkill != null && SecondrySkill != "null")
	{
		...
		while (SecondrySkill.indexOf('+') >= 0) 
		{
			//SecondrySkill = SecondrySkill.replace('+', '%2B'); old one
			SecondrySkill = SecondrySkill.replace(/[+]/g,'%2B');
		}
	}
    //Employee_ID = "10799";
    var refined_data = [];
    ...
    var site_url = "https://ppmprod.bcone.com/";
    if (Employee_ID == "NA") {
        ...
        url = url = searchResourcefromResources(RolBand, PrimarySkill, SecondrySkill); //"https://ppmprod.bcone.com/_api/Resources?$select=EmployeeID,Designation,EmployeeStatus,ResourceEmailAddress,ResourceName&$filter=RoleBand eq'" + RolBand + "' and (PrimarySkill eq'" + PrimarySkill + "' or Skill eq'" + SecondrySkill + "')"; //"_api/WorkdayResourceMaster?$select=EmployeeID,Designation,Employee_status,Work_Email&$filter=Role_Band eq'" + RolBand + "' and (PrimarySkill eq'" + PrimarySkill + "' or secondarySkill eq'" + SecondrySkill + "')";
        $.ajax({
            url: url,
            method: "GET",
            async: false,
        ...
        url = searchResourcefromResourcesUsingEmpid(Employee_ID); //"https://ppmprod.bcone.com/_api/Resources?$select=EmployeeID,Designation,EmployeeStatus,ResourceEmailAddress,ResourceName&$filter=EmployeeID eq'" + Employee_ID + "'";//"_api/WorkdayResourceMaster?$select=EmployeeID,Designation,Employee_status,Work_Email&$filter=EmployeeID eq'" + Employee_ID + "'";
        $.ajax({
            url: url,
            method: "GET",
            async: false,
...
function searchResourceWithNewService(Employee_ID, PrimarySkill, SecondrySkill, RolBand, projectStrtdate, Allocationvalue, ProjectEnddate) {
    if (PrimarySkill.indexOf('&') >= 0) {
       // PrimarySkill = PrimarySkill.replace('&', '%26'); old one
		PrimarySkill = PrimarySkill.replace(/[&]/g,'%26');
    ...
    if (SecondrySkill.indexOf('&') >= 0) {
        //SecondrySkill = SecondrySkill.replace('&', '%26');
		SecondrySkill = SecondrySkill.replace(/[&]/g,'%26');
    }
    if (SecondrySkill.indexOf('+') >= 0) {
     //   SecondrySkill = SecondrySkill.replace('&', '%26');
		SecondrySkill = SecondrySkill.replace(/[+]/g,'%2B');
